...
function checkHtmlForExternalAssets() {
    ...
    assert(!externalAssetPattern.test(indexHtml), 'index.html still contains external runtime assets.');
}
...
function main() {
    ...
    const combinedData = readJson('data.json');
    assert(Array.isArray(combinedData.products), 'data.json.products should be an array.');
    assert(Array.isArray(combinedData.logs), 'data.json.logs should be an array.');
