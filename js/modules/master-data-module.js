(function initMasterDataModule(global) {
    ...
    function buildRecordInfoCard(record) {
        ...
        const safeCreatedAt = escapeHTML(formatDateTime(record.createdAt || new Date()));
        const safeUpdatedAt = escapeHTML(formatDateTime(record.updatedAt || new Date()));
    ...
    function renderStatusSelect(containerId, inputId, value) {
        renderAntdSelect(
            ...
            inputId,
            STATUS_OPTIONS,
            { placeholder: '请选择状态', value: value || 'active' }
        );
    }

    function renderPaymentTermsSelect(containerId, inputId, value, placeholder = '请选择付款条件') {
        renderAntdSelect(
            containerId,
            inputId,
            PAYMENT_OPTIONS,
    ...
    function showAddProductModal() {
        ...
        const supplierOptions = mockData.suppliers.map(supplier => ({ value: supplier.id, label: supplier.name }));
        ...
        showModal('新增商品', formContent, function onConfirm() {
            ...
            if (!formData.get('costPrice')) { alert('请输入成本单价'); return false; }
            if (!formData.get('retailPrice')) { alert('请输入销售单价'); return false; }
        ...
        const renderCategorySelect = (value = '') => {
            renderAntdSelect('modal-category-container', 'modal-category-input', categoryOptions, {
                placeholder: '请选择分类',
        ...
        const renderSupplierSelect = (value = '') => {
            renderAntdSelect('modal-supplier-container', 'modal-supplier-input', supplierOptions, {
                placeholder: '请选择供应商',
                value: value || undefined
            });
        ...
        const applyProductDefaults = product => {
            ...
            setInputValue('#add-product-form input[name="costPrice"]', product.costPrice ?? '');
            setInputValue('#add-product-form input[name="retailPrice"]', product.retailPrice ?? '');
        };

        renderAntdSelect('modal-product-name-container', 'modal-product-choice-input', productOptions, {
            ...
            const hiddenNameInput = document.getElementById('modal-product-name-input');
    ...
    function addProduct(productData) {
        const existingProduct = mockData.products.find(product => product.name === productData.name && product.supplierId === productData.supplierId);
        ...
        if (existingProduct) {
            ...
            addLog('add', 'product', productData.name, `新增商品，数量：${productData.quantity}，成本单价：${productData.costPrice}`);
        }
    ...
    function showViewProductModal(productId) {
        ...
        const stockValue = (Number(product.costPrice) || 0) * (Number(product.stockQuantity) || 0);
        ...
        const safeStatusLabel = escapeHTML(statusMeta.label);
        const safeCreatedAt = escapeHTML(formatDateTime(product.createdAt || new Date()));
        const safeUpdatedAt = escapeHTML(formatDateTime(product.updatedAt || new Date()));
        const costPriceDisplay = escapeHTML(formatCurrencyValue(product.costPrice));
        const retailPriceDisplay = escapeHTML(formatCurrencyValue(product.retailPrice));
    ...
    function updateInventoryTable() {
        ...
        if (paginatedProducts.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">没有找到匹配的商品</td></tr>';
        ...
        paginatedProducts.forEach(product => {
            ...
            const formattedCreatedAt = formatDateTime(product.createdAt || new Date());
            const formattedUpdatedAt = formatDateTime(product.updatedAt || new Date());
            ...
            if (viewButton) {
                viewButton.addEventListener('click', () => showViewProductModal(product.id));
            }
    ...
    function showEditCompanyModal(companyId) {
        ...
        const contactPersonValue = escapeHTML(getEditableFieldValue(company.contactPerson));
        const contactPhoneValue = escapeHTML(getEditableFieldValue(company.contactPhone));
        ...
        showModal('编辑公司', content, function onConfirm() {
            ...
            const status = normalizeTextValue(document.getElementById('edit-company-status-input').value) || 'active';
        ...
        renderStatusSelect('edit-company-status-container', 'edit-company-status-input', company.status || 'active');
    }
    ...
    function updateCompanyTable() {
        ...
        paginatedCompanies.forEach(company => {
            ...
            if (viewButton) {
                viewButton.addEventListener('click', () => showViewCompanyModal(company.id));
            }
            if (editButton) {
                editButton.addEventListener('click', () => showEditCompanyModal(company.id));
            }
            tbody.appendChild(row);
        });
    ...
    function showAddSupplierModal() {
        ...
        showModal('新增供应商', content, function onConfirm() {
            ...
            const contactPhone = formData.get('contactPhone').trim();
            const email = formData.get('email').trim();
            const paymentTerms = document.getElementById('add-supplier-payment-input').value;
        ...
        configureWideFormModal('创建');
        renderAntdSelect('add-supplier-payment-container', 'add-supplier-payment-input', PAYMENT_OPTIONS, '请选择付款条件');
    }
    ...
    function showEditSupplierModal(supplierId) {
        ...
        const contactPersonValue = escapeHTML(getEditableFieldValue(supplier.contactPerson));
        const contactPhoneValue = escapeHTML(getEditableFieldValue(supplier.contactPhone));
        ...
        const paymentTermsValue = escapeHTML(getEditableFieldValue(supplier.paymentTerms));
        ...
        showModal('编辑供应商', content, function onConfirm() {
            ...
            const email = normalizeTextValue(formData.get('email'));
            const address = normalizeTextValue(formData.get('address'));
            const paymentTerms = normalizeTextValue(document.getElementById('edit-supplier-payment-input').value);
            const status = normalizeTextValue(document.getElementById('edit-supplier-status-input').value) || 'active';
            ...
            saveMockData();
            addLog('edit', 'supplier', name, `编辑供应商信息，联系人：${contactPerson}`);
            updateSupplierTable();
            alert('供应商信息已更新');
            return true;
        });
        ...
        configureWideFormModal('保存');
        renderPaymentTermsSelect('edit-supplier-payment-container', 'edit-supplier-payment-input', getEditableFieldValue(supplier.paymentTerms), '请选择付款条件');
        renderStatusSelect('edit-supplier-status-container', 'edit-supplier-status-input', supplier.status || 'active');
    }
    ...
    function showAddCustomerModal() {
        ...
        showModal('新增客户', content, function onConfirm() {
            ...
            const address = formData.get('address').trim();
            const email = formData.get('email').trim();
            const paymentTerms = document.getElementById('add-customer-payment-input').value;

            if (!name) { alert('请输入客户名称'); return false; }
            if (!contactPerson) { alert('请输入联系人'); return false; }
            if (!contactPhone) { alert('请输入联系电话'); return false; }
            if (!address) { alert('请输入客户地址'); return false; }
            if (!DOMESTIC_PHONE_REGEX.test(contactPhone)) {
                alert('请输入有效的国内联系电话（手机号或座机号）');
                return false;
            }
        ...
        configureWideFormModal('创建');
        renderAntdSelect('add-customer-payment-container', 'add-customer-payment-input', PAYMENT_OPTIONS, 'Net 30');
    }
    ...
    function showEditCustomerModal(customerId) {
        ...
        const contactPersonValue = escapeHTML(getEditableFieldValue(customer.contactPerson));
        const contactPhoneValue = escapeHTML(getEditableFieldValue(customer.contactPhone));
        ...
        const paymentTermsValue = escapeHTML(getEditableFieldValue(customer.paymentTerms));
        ...
        showModal('编辑客户', content, function onConfirm() {
            ...
            const address = normalizeTextValue(formData.get('address'));
            const email = normalizeTextValue(formData.get('email'));
            const paymentTerms = normalizeTextValue(document.getElementById('edit-customer-payment-input').value);
            const status = normalizeTextValue(document.getElementById('edit-customer-status-input').value) || 'active';

            if (!name) { alert('请输入客户名称'); return false; }
            if (!contactPerson) { alert('请输入联系人'); return false; }
            if (!contactPhone) { alert('请输入联系电话'); return false; }
            if (!address) { alert('请输入客户地址'); return false; }
            if (!paymentTerms) { alert('请选择付款条件'); return false; }
            if (!DOMESTIC_PHONE_REGEX.test(contactPhone)) {
                alert('请输入有效的国内联系电话（手机号或座机号）');
                return false;
            }
            ...
            saveMockData();
            addLog('edit', 'customer', name, `编辑客户信息，联系人：${contactPerson}`);
            updateCustomerTable();
            alert('客户信息已更新');
            return true;
        });
        ...
        configureWideFormModal('保存');
        renderPaymentTermsSelect('edit-customer-payment-container', 'edit-customer-payment-input', getEditableFieldValue(customer.paymentTerms), '请选择付款条件');
        renderStatusSelect('edit-customer-status-container', 'edit-customer-status-input', customer.status || 'active');
    }
    ...
    function updateCustomerTable() {
        ...
        paginatedCustomers.forEach(customer => {
            ...
            const editButton = row.querySelector('[data-action="edit"]');
            if (viewButton) {
                viewButton.addEventListener('click', () => showViewCustomerModal(customer.id));
            }
            if (editButton) {
                editButton.addEventListener('click', () => showEditCustomerModal(customer.id));
            }
            tbody.appendChild(row);
        });
    ...
    function updateSupplierTable() {
        ...
        paginatedSuppliers.forEach(supplier => {
            ...
            const editButton = row.querySelector('[data-action="edit"]');
            if (viewButton) {
                viewButton.addEventListener('click', () => showViewSupplierModal(supplier.id));
            }
            if (editButton) {
                editButton.addEventListener('click', () => showEditSupplierModal(supplier.id));
            }
            tbody.appendChild(row);
        });
