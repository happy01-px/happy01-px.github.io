(function initLogsModule(global) {
  ...
  function renderLogsTable() {
    ...
    const userFilter = document.getElementById("log-filter-user")
      ? document.getElementById("log-filter-user").value.toLowerCase().trim()
      : "";
    ...
    const searchFilter = document.getElementById("log-filter-search")
      ? document.getElementById("log-filter-search").value.toLowerCase().trim()
      : "";
    ...
    let filteredLogs = logsData.filter((log) => {
      ...
      if (searchFilter && !log.objectName.toLowerCase().includes(searchFilter))
        return false;
    ...
    if (paginatedLogs.length === 0) {
      renderAntdEmptyTableRow(logsTableBody, 6, "没有找到匹配的日志记录");
