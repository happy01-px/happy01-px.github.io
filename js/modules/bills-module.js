(function initBillsModule(global) {
    ...
    const BILL_STATUS_META = Object.freeze({
        ...
        partial_paid: { label: '部分付款', badgeClass: 'bg-yellow-100 text-yellow-700' },
    ...
    function getCurrentUserInitialForBills() {
        ...
        return global.escapeHTML ? global.escapeHTML(name.charAt(0) || '张') : (name.charAt(0) || '张');
    }

    function getStatusBadgeHtml(status) {
        const meta = BILL_STATUS_META[normalizeBillStatus(status)] || BILL_STATUS_META.pending_check;
    ...
    function convertAmountToChineseUpperForBills(amount) {
        ...
        function convertInteger(num) {
            ...
            return result.join('').replace(/零+/g, '零').replace(/零(万|亿|兆)/g, '$1');
        }
        ...
        return `人民币${integerText}元${fractionText || '整'}`.replace(/零角零分$/, '整').replace(/零分$/, '');
    }

    function getStatementRecords() {
        return (global.normalizeList ? global.normalizeList(global.mockData?.bills) : []).filter((record) => record?.recordType === 'statement-v1');
    }

    function getOutstandingAmount(statement) {
        const totalAmount = Number(statement?.totalAmount) || 0;
        const paidAmount = (global.normalizeList ? global.normalizeList(statement?.payments) : []).reduce((sum, payment) => sum + (Number(payment?.payAmount) || 0), 0);
    ...
    function isPurchaseDeliveryNote(note) {
        return Boolean(note) && !isSalesDeliveryNote(note) && (note.supplierId || note.orderId);
    }
    ...
    function getStatementPartyFilterValue(statement) {
        return String(statement?.partyId || statement?.partyNameSnapshot || '').trim();
    }
    ...
    function findSupplierRecord(reference) {
        ...
        return global.normalizeList(global.mockData?.suppliers).find((supplier) =>
            String(supplier.id || '').trim().toLowerCase() === normalizedReference ||
            String(supplier.name || '').trim().toLowerCase() === normalizedReference
        ) || null;
    }
    ...
    function mapLegacySupplierBillToStructured(record, existingStatements) {
        const amount = Number(String(record.amount || '').replace(/[^\d.-]/g, '')) || 0;
        const createdAt = record.createdAt || global.getLocalISOString();
        const period = parseLegacyStatementPeriod(record.period, createdAt);
        const supplier = findSupplierRecord(record.supplierId || record.supplierName);
        return {
            ...
            partyNameSnapshot: record.supplierName || supplier?.name || '未命名供应商',
            companyId: '',
            companyNameSnapshot: '',
    ...
    function migrateBillsData(records) {
        ...
        let changed = false;
        const nextRecords = [];
        const existingStructuredStatements = global.normalizeList(records).filter((record) => record?.recordType === 'statement-v1');
        ...
        global.normalizeList(records).forEach((record) => {
            if (record?.recordType !== 'statement-v1') {
                if (record && (record.supplierName || record.supplierId)) {
                    const migratedLegacyRecord = mapLegacySupplierBillToStructured(record, existingStructuredStatements);
            ...
            if (record.statementType === 'customer') {
                const sourceIds = global.normalizeList(record.sourceDocumentIds)
                    .map((id) => String(id || '').trim())
                    .filter(Boolean);

                if (sourceIds.length > 0 && sourceIds.every((id) => purchaseDeliveryIds.has(id))) {
                    changed = true;
                    return;
                }
            }
            ...
            if (record.statementType === 'supplier') {
                const supplier = findSupplierRecord(record.partyId || record.partyNameSnapshot);
                if (supplier) {
                    nextRecord = {
                        ...record,
                        partyId: record.partyId || supplier.id,
                        partyNameSnapshot: record.partyNameSnapshot || supplier.name,
                        contactNameSnapshot: record.contactNameSnapshot || supplier.contactPerson || '',
                        contactPhoneSnapshot: record.contactPhoneSnapshot || supplier.contactPhone || '',
                        partyAddressSnapshot: record.partyAddressSnapshot || supplier.address || ''
                    };
    ...
    function createBillsDemoSourceData() {
        return [
            ...
            },
            {
                ...
                status: 'created',
                totalAmount: 20997,
                notes: '苏宁易购对账测试单 - iPhone 13 Pro x 1, MacBook Air M2 x 1, Apple Watch x 1',
                companyId: 'CO001',
                companyName: '化工',
                companyAddress: '上海市浦东新区张江高科技园区',
                companyPhone: '13800138001',
    ...
    async function ensureBillsDemoSourceData() {
        ...
        const existingIds = new Set(global.normalizeList(global.mockData.deliveryNotes).map((note) => String(note.id || '').trim()));
        const demoRecords = createBillsDemoSourceData().filter((record) => !existingIds.has(record.id));
        if (!demoRecords.length) return false;

        global.mockData.deliveryNotes = demoRecords.concat(global.normalizeList(global.mockData.deliveryNotes));
    ...
    function mapDeliveryNoteToStructured(note, index) {
        const details = (global.normalizeList ? global.normalizeList(note.details) : []).map((detail, detailIndex) => ({
        ...
        const amount = details.reduce((sum, item) => sum + (Number(item.lineAmount) || 0), 0);
    ...
    async function ensureBillsSeedData() {
        ...
        const currentBills = global.normalizeList ? global.normalizeList(global.mockData.bills) : [];
        const migrated = migrateBillsData(currentBills);
        const nextBills = migrated.records.slice();
        const structured = nextBills.filter((record) => record?.recordType === 'statement-v1');
        let changed = migrated.changed || nextBills.length !== currentBills.length;

        if (!structured.some((record) => record.statementType === 'customer')) {
            global.normalizeList(global.mockData.deliveryNotes).filter(isSalesDeliveryNote).forEach((note, index) => {
    ...
    function normalizeBillsSectionCopy() {
        ...
        const labels = section.querySelectorAll('.bg-white.rounded-lg.shadow-card.p-4.mb-6 label');
        ...
        const titles = ['对账单编号', getBillsMeta(state.activeTab).partyLabel, '对账期间', '账单金额', '状态', '创建与更新', '操作'];
    ...
    function applyBillsListVisualParity() {
        ...
        if (addButton) {
            addButton.className = 'bg-primary hover:bg-primary-dark text-white px-6 py-2 rounded-lg flex items-center transition-all-300';
            addButton.parentElement?.classList.add('bills-list-toolbar');
        }
    }

    function closeBillsModal() {
        const { overlay, panel, content, cancel, confirm, footer } = getBillsModalElements();
        ...
        if (cancel) {
            cancel.className = 'bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg mr-2 transition-all-300';
            cancel.textContent = '取消';
            cancel.onclick = null;
        }
        if (confirm) {
            confirm.className = 'bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300';
    ...
    function openBillsModal(config) {
        ...
        if (!elements.overlay || !elements.panel || !elements.content || !elements.title || !elements.footer) {
            return;
        }

        state.modalCloseHandler = typeof config.onClose === 'function' ? config.onClose : null;

        elements.title.textContent = config.title || '查看对账单';
        elements.panel.className = config.panelClassName || 'bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4';
        ...
        if (footer) {
            footer.className = config.footerClassName || 'flex items-center justify-between p-4 border-t border-gray-200';
    ...
    function getStatementsForTab(tabKey) {
        if (tabKey === 'payment') {
            return getStatementRecords().filter((record) => ['pending_payment', 'partial_paid'].includes(normalizeBillStatus(record.status)));
        }
        return getStatementRecords().filter((record) => record.statementType === tabKey);
    }

    function getPartyOptions() {
        if (state.activeTab === 'supplier') {
            return global.normalizeList(global.mockData?.suppliers).map((supplier) => ({
                value: supplier.id,
                label: supplier.name
            }));
        }
        ...
        if (state.activeTab === 'customer') {
            return global.normalizeList(global.mockData?.customers).map((customer) => ({
                value: customer.id,
                label: customer.name
            }));
        }
    ...
    function renderBillPartyFilter() {
        ...
        if (!container || !input || typeof global.renderAntdSelect !== 'function') return;
        ...
        global.renderAntdSelect(
            ...
            getPartyOptions(),
            {
                placeholder: state.activeTab === 'supplier' ? '全部供应商' : state.activeTab === 'customer' ? '全部客户' : '全部对象',
    ...
    function renderBillStatusFilter() {
        ...
        const statuses = state.activeTab === 'payment'
            ? ['pending_payment', 'partial_paid']
            : ['pending_check', 'pending_payment', 'partial_paid', 'paid', 'cancelled'];

        global.renderAntdSelect(
            'bills-filter-status-container',
            'bills-filter-status',
            [{ value: '', label: '全部状态' }].concat(statuses.map((status) => ({
                value: status,
    ...
    function renderBillSearchInput() {
        ...
        if (!container || !input || typeof global.renderAntdInput !== 'function') return;
    ...
    function getStatementDisplayName(statement) {
        return statement.partyNameSnapshot || (state.activeTab === 'supplier' ? '未命名供应商' : '未命名客户');
    }

    function getBillUserInitial(statement) {
        if (statement.updatedByName) {
            return global.escapeHTML(String(statement.updatedByName).charAt(0) || '张');
        }
    ...
    function getBillTimeCellHtml(statement) {
        ...
        return `
            <div class="space-y-1 text-sm text-gray-600 whitespace-nowrap">
                <div class="flex items-center gap-2">
                    <span>创建时间:</span>
                    <span class="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center text-sm font-semibold">${initial}</span>
                    <span class="px-3 py-1 rounded-full bg-blue-100 text-blue-700">${formatBillDateTime(statement.createdAt)}</span>
                </div>
                <div class="flex items-center gap-2">
                    <span>更新时间:</span>
                    <span class="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center text-sm font-semibold">${initial}</span>
                    <span class="px-3 py-1 rounded-full bg-blue-100 text-blue-700">${formatBillDateTime(statement.updatedAt)}</span>
                </div>
            </div>
        `;
    }
    ...
    function getBillRowActions(record) {
        const paymentDisabled = ['paid', 'cancelled'].includes(normalizeBillStatus(record.status));
        return `
    ...
    function renderBillRows(records) {
        return records.map((record) => `
            <tr>
                ...
                <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">${global.escapeHTML(getStatementDisplayName(record))}</td>
                <td class="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">${global.escapeHTML(formatStatementPeriod(record.periodStart, record.periodEnd))}</td>
                ...
                <td class="px-6 py-4 align-top">${getBillTimeCellHtml(record)}</td>
                <td class="px-6 py-4 bills-action-cell align-top">${getBillRowActions(record)}</td>
            </tr>
        `).join('');
    }
    ...
    function getFilteredStatements() {
        ...
        const partyValue = document.getElementById('bills-filter-supplier')?.value || '';
        const statusValue = document.getElementById('bills-filter-status')?.value || '';
        const startValue = document.getElementById('bills-filter-date-start')?.value || '';
        const endValue = document.getElementById('bills-filter-date-end')?.value || '';
        const searchValue = String(document.getElementById('bills-filter-search')?.value || '').trim().toLowerCase();

        return statements
            .filter((record) => !partyValue || getStatementPartyFilterValue(record) === partyValue)
            .filter((record) => !statusValue || normalizeBillStatus(record.status) === statusValue)
            .filter((record) => {
                if (!startValue && !endValue) return true;
                const createdAt = normalizeBillDate(record.statementDate || record.createdAt);
                if (!createdAt) return true;
                const dateOnly = formatBillDateOnly(createdAt);
                return (!startValue || dateOnly >= startValue) && (!endValue || dateOnly <= endValue);
            })
            .filter((record) => {
            ...
            .sort((a, b) => {
                const timeA = normalizeBillDate(a.updatedAt || a.createdAt)?.getTime() || 0;
                const timeB = normalizeBillDate(b.updatedAt || b.createdAt)?.getTime() || 0;
                return timeB - timeA;
            });
    }
    ...
    function updateBillsTableOverride() {
        ...
        if (!currentPageRows.length) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="7" class="bills-empty-state">${global.escapeHTML(getBillsMeta(state.activeTab).emptyText)}</td>
                </tr>
            `;
        } else {
        ...
        if (typeof global.renderPaginationControl === 'function') {
            global.renderPaginationControl('bills-pagination-container', 'bills', () => {
                updateBillsTableOverride();
            });
        }
    }
    ...
    function parseBillsRouteHash(hashValue) {
        ...
        if (rawHash.startsWith('#/bills/view/')) {
            const statementId = decodeURIComponent(rawHash.slice('#/bills/view/'.length));
    ...
    function buildBillsRouteState(route, options) {
        ...
        if (route.type === 'view') {
            return {
                ...
                returnTab: config.returnTab || state.activeViewReturnTab || state.activeTab || 'customer'
            };
        }
    ...
    function returnToPreviousBillsLevel(statementId, fallbackTab) {
        ...
        if (previousRoute?.type === 'view' && previousRoute.statementId && previousRoute.statementId !== statementId) {
        ...
        navigateToBillsRoute({ type: 'list', tab: fallbackTab }, { tab: fallbackTab });
    }

    function navigateToBillsRoute(route, options) {
        const config = options || {};
        ensureBillsRouteSections();
        const currentRouteState = parseBillsRouteHash(global.location.hash) || state.currentBillsRoute;
        ...
        if (route.type === 'view') {
            ...
            showBillsSection('bills-view', `#/bills/view/${encodeURIComponent(route.statementId)}`);
        }
    }
    ...
    function getDraftSourceOccupancyEntries(formData) {
        if (!formData?.statementType || !formData.partyId || !formData.periodStart || !formData.periodEnd) {
            return [];
        }
        ...
        if (formData.statementType === 'supplier') {
            const supplier = global.normalizeList(global.mockData?.suppliers).find((item) => item.id === formData.partyId);
            const purchaseSources = global.normalizeList(global.mockData?.deliveryNotes)
                .filter(isPurchaseDeliveryNote)
                .filter((note) => note.supplierId === formData.partyId)
                .filter((note) => {
                    const date = formatBillDateOnly(note.deliveryDate || note.expectedDate || note.createdAt);
                    return date >= formData.periodStart && date <= formData.periodEnd;
                })
                .map((note) => ({
                    id: String(note.id || '').trim(),
                    sourceNo: note.orderId || note.id,
                    bizDate: formatBillDateOnly(note.deliveryDate || note.expectedDate || note.createdAt),
                    sourceType: 'purchase_delivery_note'
                }));

            const inboundSources = global.normalizeList(global.stockMovementData)
                .filter((record) => record.type === 'inbound')
                .filter((record) => record.supplierId === formData.partyId || (supplier?.name && record.supplierName === supplier.name))
                .filter((record) => {
            ...
            sourcePool = global.normalizeList(global.mockData?.deliveryNotes)
                .filter(isSalesDeliveryNote)
                .filter((note) => note.companyId === formData.companyId)
                .filter((note) => note.customerId === formData.partyId)
                .filter((note) => {
                    const date = formatBillDateOnly(note.deliveryDate || note.issueDate || note.createdAt);
                    return date >= formData.periodStart && date <= formData.periodEnd;
                })
                .map((note) => ({
                    id: String(note.id || '').trim(),
                    sourceNo: note.orderNo || note.id,
                    bizDate: formatBillDateOnly(note.deliveryDate || note.issueDate || note.createdAt),
                    sourceType: 'delivery_note'
                }));
        }
        ...
        return getStatementRecords()
            ...
            .filter((record) => normalizeBillStatus(record.status) !== 'cancelled')
            .map((record) => {
                const matchedSources = global.normalizeList(record.sourceDocumentIds)
                    .map((id) => String(id || '').trim())
            ...
            .filter(Boolean)
            .sort((a, b) => {
                const timeA = normalizeBillDate(a.statement.updatedAt || a.statement.createdAt)?.getTime() || 0;
                const timeB = normalizeBillDate(b.statement.updatedAt || b.statement.createdAt)?.getTime() || 0;
                return timeB - timeA;
            });
    }

    function openOccupiedStatementsModal(formData, occupiedEntries) {
        const occupiedSourceCount = occupiedEntries.reduce((sum, entry) => sum + entry.matchedSources.length, 0);
        const content = `
            <div class="space-y-3 text-sm text-gray-700">
                <p>当前条件下匹配到的来源单据里，有 <strong>${global.escapeHTML(String(occupiedSourceCount))}</strong> 条已经被其他对账单占用，所以当前无法继续生成。</p>
                <div class="space-y-3 max-h-80 overflow-y-auto pr-1">
                    ${occupiedEntries.map((entry) => `
                        <div class="rounded-lg bg-gray-50 border border-gray-200 p-4 space-y-2">
                            <div class="flex items-start justify-between gap-4">
                                <div class="min-w-0 space-y-1">
                                    <div class="text-sm font-semibold text-gray-900">${global.escapeHTML(entry.statement.id)}</div>
                                    <div class="text-xs text-gray-500">${global.escapeHTML(entry.statement.partyNameSnapshot || '-')} · ${global.escapeHTML(formatStatementPeriod(entry.statement.periodStart, entry.statement.periodEnd))}</div>
                                    <div class="text-xs text-gray-500">状态：${global.escapeHTML(BILL_STATUS_META[normalizeBillStatus(entry.statement.status)]?.label || '-')}</div>
                                </div>
                                <button type="button" class="text-primary hover:text-primary-dark whitespace-nowrap" data-occupied-view="${global.escapeHTML(entry.statement.id)}">查看对账单</button>
                            </div>
                            <div class="text-xs text-gray-600 break-all">占用来源单：${global.escapeHTML(entry.matchedSources.map((item) => item.sourceNo || item.id).join('、'))}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
        ...
        openBillsModal({
            ...
            content,
            rightButtons: [
                { label: '关闭', className: 'bills-outline-button', onClick: closeBillsModal }
            ]
        });

        document.querySelectorAll('[data-occupied-view]').forEach((button) => {
            button.addEventListener('click', () => {
                const statement = getStatementRecords().find((item) => item.id === button.getAttribute('data-occupied-view'));
                if (!statement) return;
                state.pendingDraft = formData;
                closeBillsModal();
                openStatementViewModal(statement, { returnTab: formData.statementType || state.activeTab || 'customer' });
            });
        });
    }

    function createStatementFromDraft(formData) {
        const statement = formData.statementType === 'supplier'
    ...
    function renderBillsCreateRoute(draft) {
            ...
            <div class="bills-route-shell">
                <div class="bills-route-header">
                    <div class="bills-route-title">
                        <span class="text-sm text-gray-500">对账单系统 / 新增对账单</span>
                        <h2 class="text-2xl font-bold text-gray-800">新增对账单</h2>
                        <p class="text-gray-600">填写对账条件后，系统会根据公司、对象和时间范围自动生成正式对账单。</p>
                    </div>
                    <div class="bills-route-actions">
                        <button id="bill-create-back-btn" type="button" class="bills-outline-button">返回对账单列表</button>
                    </div>
                </div>
                <div class="bills-route-card">
                    <div class="bills-route-card-body space-y-5">
                        <div class="bills-modal-form-grid">
                            ...
                            <div class="bills-field">
                                <label>${global.escapeHTML(currentDraft.statementType === 'supplier' ? '供应商' : '客户')}</label>
                            ...
                            <div class="bills-field justify-center">
                        ...
                        <p class="bills-muted-note">如果当前条件下已经存在相同账期的对账单，系统会先提示你查看已有对账单，避免重复生成。</p>
                        <div class="bills-route-actions justify-end">
                            ...
                            <button id="bill-create-submit-btn" type="button" class="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300">生成对账单</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        document.getElementById('bill-create-back-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list' }, { tab: currentDraft.statementType }));
        document.getElementById('bill-create-cancel-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list' }, { tab: currentDraft.statementType }));
        document.getElementById('bill-create-submit-btn')?.addEventListener('click', () => {
            const formData = getCreateDraftFromModal();
            state.pendingDraft = formData;
            if (!formData.companyId || !formData.partyId || !formData.periodStart || !formData.periodEnd) {
                alert('请先完整选择公司、对象和时间范围');
                return;
            }
            ...
            const duplicate = findDuplicateStatement(formData);
            if (duplicate) {
                openDuplicatePromptModal(formData, duplicate);
                return;
            }
    ...
    function renderBillsCreateRoute(draft) {
        ...
        const partyLabel = currentDraft.statementType === 'supplier'
            ? '供应商'
        ...
        document.getElementById('bill-create-back-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list' }, { tab: listTab }));
        document.getElementById('bill-create-cancel-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list' }, { tab: listTab }));
        document.getElementById('bill-create-submit-btn')?.addEventListener('click', () => {
            const formData = getCreateDraftFromModal();
            state.pendingDraft = formData;

            if (!formData.statementType || !formData.companyId || !formData.partyId || !formData.statementDate || !formData.periodStart || !formData.periodEnd || !formData.taxRate) {
    ...
    function renderBillsViewRoute(statementId, options) {
        ...
        const statement = getStatementRecords().find((item) => item.id === statementId);
        const returnTab = options?.returnTab || state.activeViewReturnTab || statement?.statementType || 'customer';
        ...
        if (!statement) {
            section.innerHTML = `
                <div class="bills-route-shell">
                    <div class="bills-route-header">
                        <div class="bills-route-title">
                            <span class="text-sm text-gray-500">对账单系统 / 查看对账单</span>
                            <h2 class="text-2xl font-bold text-gray-800">未找到对账单</h2>
                            <p class="text-gray-600">这个对账单可能已经被删除，或者当前数据尚未同步。</p>
                        </div>
                        <div class="bills-route-actions">
                    ...
                    <div class="bills-route-card">
                        <div class="bills-route-empty">未找到编号为 ${global.escapeHTML(statementId)} 的对账单。</div>
                    </div>
                </div>
            `;
            document.getElementById('bill-view-missing-back-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list', tab: returnTab }));
            return;
        }
            ...
            <div class="bills-route-shell">
                <div class="bills-route-header">
                    <div class="bills-route-title">
                        ...
                        <p class="text-gray-600">${global.escapeHTML(getBillsMeta(statement.statementType).label)} · ${global.escapeHTML(statement.partyNameSnapshot || '-')} · ${global.escapeHTML(formatStatementPeriod(statement.periodStart, statement.periodEnd))}</p>
                    </div>
                    <div class="bills-route-actions">
                        <button id="bill-view-back-btn" type="button" class="bills-outline-button">返回对账单列表</button>
                        <button id="bill-view-export-excel-btn" type="button" class="bills-outline-button">导出 Excel</button>
                        <button id="bill-view-export-pdf-btn" type="button" class="bills-outline-button">导出 PDF</button>
                        <button id="bill-view-register-payment-btn" type="button" class="bills-outline-button">登记付款</button>
                        <button id="bill-view-confirm-export-btn" type="button" class="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300">确认导出</button>
                    </div>
                </div>
                <div class="bills-route-card">
                    <div class="bills-route-card-body">
                        <div class="bills-route-toolbar">
                            ...
                            <button id="bill-view-cancel-btn" type="button" class="inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-2 font-semibold text-red-700 transition-all-300 hover:bg-red-100 hover:border-red-300">作废对账单</button>
                        </div>
        ...
        document.getElementById('bill-view-back-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list', tab: returnTab }));
        document.getElementById('bill-view-export-excel-btn')?.addEventListener('click', () => exportStatementAsExcel(statement));
        document.getElementById('bill-view-export-pdf-btn')?.addEventListener('click', () => exportStatementAsPdf(statement));
        document.getElementById('bill-view-register-payment-btn')?.addEventListener('click', () => openPaymentModal(statement));
        document.getElementById('bill-view-confirm-export-btn')?.addEventListener('click', () => {
            const nextStatement = normalizeBillStatus(statement.status) === 'pending_check'
                ? { ...statement, status: 'pending_payment', updatedAt: global.getLocalISOString() }
                : statement;
            if (nextStatement !== statement) {
                saveStatementRecord(nextStatement, false);
            }
            exportStatementAsExcel(nextStatement);
            exportStatementAsPdf(nextStatement);
        });
        document.getElementById('bill-view-cancel-btn')?.addEventListener('click', () => {
            openCancelStatementConfirmModal(statement);
        });
    }

    function buildStatementPreviewMarkup(statement) {
        const sortedDetails = global.normalizeList(statement.details)
            .map((detail, index) => ({ detail, index }))
            .sort((left, right) => {
                const leftDate = formatBillDateOnly(left.detail?.bizDate || '');
                const rightDate = formatBillDateOnly(right.detail?.bizDate || '');
                if (leftDate !== rightDate) {
                    return leftDate.localeCompare(rightDate);
                }

                const leftSourceNo = String(left.detail?.sourceNo || '');
                const rightSourceNo = String(right.detail?.sourceNo || '');
                if (leftSourceNo !== rightSourceNo) {
                    return leftSourceNo.localeCompare(rightSourceNo);
                }
        ...
        const detailRows = sortedDetails.map((detail, index) => `
            <tr>
                <td>${index + 1}</td>
        ...
        const arrearsRows = global.normalizeList(statement.arrears).map((item) => `
            <tr>
        ...
        const paymentRows = global.normalizeList(statement.payments).map((item) => `
            <tr>
                ...
                <div class="bills-modal-section">
                    ...
                    <div class="bills-modal-section-body overflow-x-auto">
                        <table class="bills-inline-table">
                            ...
                            <tbody>${detailRows || '<tr><td colspan="10" class="text-center text-gray-400">暂无明细</td></tr>'}</tbody>
                        </table>
                    </div>
                </div>
                    ...
                    <div class="bills-modal-section">
                        ...
                        <div class="bills-modal-section-body overflow-x-auto">
                            <table class="bills-inline-table">
                                ...
                                <tbody>${arrearsRows || '<tr><td colspan="2" class="text-center text-gray-400">暂无往期欠款</td></tr>'}</tbody>
                            </table>
                        </div>
                    </div>
                    <div class="bills-modal-section">
                        ...
                        <div class="bills-modal-section-body overflow-x-auto">
                            <table class="bills-inline-table">
                                ...
                                <tbody>${paymentRows || '<tr><td colspan="4" class="text-center text-gray-400">暂无付款记录</td></tr>'}</tbody>
                            </table>
                        </div>
                    </div>
                </div>
    ...
    function exportStatementAsExcel(statement) {
        const blob = new Blob([buildStatementExportHtml(statement)], { type: 'application/vnd.ms-excel;charset=utf-8;' });
    ...
    async function exportStatementAsPdf(statement) {
        try {
            ...
            alert(`PDF 导出失败：${error?.message || '请检查本地导出服务是否正常运行'}`);
            return false;
        }
    }
    ...
    function saveStatementRecord(statement, reopenView) {
        ...
        const nextBills = bills.map((item) => item.id === statement.id ? statement : item);
        global.mockData.bills = nextBills;
        if (typeof global.saveMockData === 'function') {
            global.saveMockData();
        }
        updateBillsTableOverride();
        const isViewingCurrentStatement = state.activeViewStatementId === statement.id && !document.getElementById('bills-view')?.classList.contains('hidden');
        if (reopenView || isViewingCurrentStatement) {
            openStatementViewModal(statement, { returnTab: state.activeViewReturnTab || statement.statementType });
        }
    }
    ...
    function openPaymentModal(statement) {
        ...
        openBillsModal({
            ...
            rightButtons: [
                ...
                {
                    label: '确认',
                    className: 'bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300',
                    onClick: function () {
                        const amount = Number(document.getElementById('bill-payment-amount').value || 0);
                        const payDate = document.getElementById('bill-payment-date').value;
                        const payMethod = document.getElementById('bill-payment-method').value || 'bank_transfer';
                        const remark = document.getElementById('bill-payment-remark').value || '';
                        ...
                        if (amount > outstandingAmount) {
                            alert(`付款金额不能大于未付金额 ${formatBillCurrency(outstandingAmount)}`);
                            return;
                        }
                        const nextStatement = {
                            ...statement,
                        ...
                        nextStatement.status = getOutstandingAmount(nextStatement) > 0 ? 'partial_paid' : 'paid';
                        closeBillsModal();
    ...
    function renderBillStatusSelect(statement) {
        ...
        global.renderAntdSelect(
            ...
            'bill-view-status',
            Object.keys(BILL_STATUS_META).map((key) => ({ value: key, label: BILL_STATUS_META[key].label })),
            { placeholder: '修改状态', value: normalizeBillStatus(statement.status) },
            (value) => {
                const nextStatement = { ...statement, status: value, updatedAt: global.getLocalISOString() };
                saveStatementRecord(nextStatement, true);
            }
        );
    }
    ...
    function openStatementViewModal(statement, options) {
        ...
        const occupiedEntries = global.normalizeList(state.occupiedViewContext?.entries);
        const targetInOccupiedContext = occupiedEntries.some((entry) => entry.statementId === statement.id);
        const shouldPreserveOccupiedContext = options?.preserveOccupiedContext === true
        ...
        if (state.occupiedViewContext && targetInOccupiedContext) {
            state.occupiedViewContext.activeStatementId = options?.occupiedActiveStatementId || statement.id;
        }

        state.activeViewReturnTab = options?.returnTab || state.activeTab || statement.statementType || 'customer';
        navigateToBillsRoute({ type: 'view', statementId: statement.id }, { returnTab: state.activeViewReturnTab });
    }

    function buildStatementFromDetails(formData, company, party, details, sourceNumbers, sourceIds) {
        const currentAmount = roundCurrency(details.reduce((sum, item) => sum + (Number(item.lineAmount) || 0), 0));
        ...
        if (formData.includeArrears) {
            arrears = getStatementRecords()
                .filter((record) => record.statementType === formData.statementType)
                .filter((record) => record.companyId === formData.companyId && record.partyId === formData.partyId)
                .filter((record) => ['pending_payment', 'partial_paid'].includes(normalizeBillStatus(record.status)))
                .filter((record) => formatBillDateOnly(record.periodEnd) < formData.periodStart)
                .map((record) => ({
            ...
            arrearsAmount = roundCurrency(arrears.reduce((sum, item) => sum + item.amount, 0));
        }

        const totalAmount = roundCurrency(amountWithTax + arrearsAmount);
        const prefix = formData.statementType === 'supplier' ? 'SST' : 'CST';
        const allStatements = getStatementRecords().filter((record) => record.statementType === formData.statementType);
    ...
    function buildCustomerStatementFromForm(formData) {
        const company = global.normalizeList(global.mockData?.companies).find((item) => item.id === formData.companyId);
        const party = global.normalizeList(global.mockData?.customers).find((item) => item.id === formData.partyId);
        if (!company || !party) return null;

        const occupiedSourceIds = getActiveSourceDocumentIdSet('customer');
        const notes = global.normalizeList(global.mockData?.deliveryNotes)
            .filter(isSalesDeliveryNote)
            .filter((note) => note.companyId === formData.companyId)
            .filter((note) => note.customerId === formData.partyId)
            .filter((note) => {
        ...
        return buildStatementFromDetails(formData, company, party, details, notes.map((note) => note.orderNo || note.id), notes.map((note) => note.id));
    }

    function buildSupplierStatementFromForm(formData) {
        const company = global.normalizeList(global.mockData?.companies).find((item) => item.id === formData.companyId);
        const party = global.normalizeList(global.mockData?.suppliers).find((item) => item.id === formData.partyId);
        if (!company || !party) return null;

        const productMap = new Map(global.normalizeList(global.mockData?.products).map((item) => [item.id, item]));
            ...
            .forEach((note) => {
                ...
                global.normalizeList(note.details).forEach((detail) => {
                    ...
                    details.push({
                        ...
                        quantity: Number(detail.quantity) || 0,
                        unitPrice: Number(detail.unitPrice) || 0,
                        lineAmount: Number(detail.totalAmount) || roundCurrency((Number(detail.quantity) || 0) * (Number(detail.unitPrice) || 0)),
        ...
        global.normalizeList(global.stockMovementData)
            .filter((record) => record.type === 'inbound')
            .filter((record) => record.supplierId === formData.partyId || record.supplierName === party.name)
            .filter((record) => {
                const date = formatBillDateOnly(record.createdAt || record.updatedAt);
                return date >= formData.periodStart && date <= formData.periodEnd;
            })
            .filter((record) => !occupiedSourceIds.has(String(record.id || '').trim()))
            .forEach((record) => {
                ...
                details.push({
                    ...
                    lineAmount: roundCurrency((Number(record.quantity) || 0) * (Number(record.price) || 0)),
                    remark: record.remark || '',
                    sortOrder: details.length + 1
                });
            });

        return buildStatementFromDetails(formData, company, party, details, sourceNumbers, sourceIds);
    }

    function findDuplicateStatement(formData) {
        return getStatementRecords().find((record) =>
    ...
    function persistStatement(statement) {
        global.mockData.bills = global.normalizeList(global.mockData.bills).concat([statement]);
        if (typeof global.saveMockData === 'function') {
            global.saveMockData();
        }
    ...
    function getCreateDraftSourceCandidates(statementType, companyId, partyId) {
        if (statementType === 'supplier') {
            const occupiedSourceIds = getActiveSourceDocumentIdSet('supplier');
            const purchaseCandidates = global.normalizeList(global.mockData?.deliveryNotes)
                .filter(isPurchaseDeliveryNote)
                .filter((note) => !partyId || note.supplierId === partyId)
                .filter((note) => !occupiedSourceIds.has(String(note.id || '').trim()))
                .map((note) => ({
                    partyId: note.supplierId || '',
                    date: formatBillDateOnly(note.deliveryDate || note.expectedDate || note.createdAt)
                }));

            const inboundCandidates = global.normalizeList(global.stockMovementData)
                .filter((record) => record.type === 'inbound')
            ...
            return purchaseCandidates.concat(inboundCandidates).filter((item) => item.partyId && item.date !== '-');
        }

        const occupiedSourceIds = getActiveSourceDocumentIdSet('customer');
        return global.normalizeList(global.mockData?.deliveryNotes)
            .filter(isSalesDeliveryNote)
            ...
            .map((note) => ({
                partyId: note.customerId || '',
                date: formatBillDateOnly(note.deliveryDate || note.issueDate || note.createdAt)
            }))
    ...
    function renderCreateBillDatePicker(containerId, inputId, value) {
        ...
        const App = () => {
            const [pickerValue, setPickerValue] = React.useState(value ? dayjs(value) : null);
    ...
    function renderCreateBillDatePickers(draft) {
        renderCreateBillDatePicker('bill-create-date-container', 'bill-create-date', draft.statementDate || '');
        renderCreateBillDatePicker('bill-create-period-start-container', 'bill-create-period-start', draft.periodStart || '');
        renderCreateBillDatePicker('bill-create-period-end-container', 'bill-create-period-end', draft.periodEnd || '');
    }
    ...
    function getCreateDraftFromModal() {
        return {
            ...
            periodStart: document.getElementById('bill-create-period-start').value || '',
            periodEnd: document.getElementById('bill-create-period-end').value || '',
            taxRate: document.getElementById('bill-create-tax-rate').value || '',
            includeArrears: !!document.getElementById('bill-create-include-arrears')?.checked
        };
    }
    ...
    function createStatementFromDraft(formData) {
        ...
        if (!statement.details.length) {
            alert('当前条件下没有可生成的对账明细，可能是没有匹配单据，或这些单据已被其他对账单占用');
            return;
        }
        persistStatement(statement);
        state.pendingDraft = null;
    ...
    function openDuplicatePromptModal(formData, duplicate) {
        openBillsModal({
            ...
            `,
            rightButtons: [
                { label: '取消', className: 'bills-outline-button', onClick: closeBillsModal },
                { label: '查看已有对账单', className: 'bills-outline-button', onClick: function () {
                    state.pendingDraft = formData;
                    closeBillsModal();
                    openStatementViewModal(duplicate, { returnTab: state.activeTab });
                } },
                { label: '返回修改条件', className: 'bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300', onClick: function () {
                    closeBillsModal();
    ...
    function renderCreateFormSelects(draft) {
        ...
        global.renderAntdSelect('bill-create-type-container', 'bill-create-type', [
            { value: 'customer', label: '客户对账单' },
            { value: 'supplier', label: '供应商对账单' }
        ], { placeholder: '选择对账类型', value: draft.statementType || 'customer' }, function (value) {
            const nextDraft = { ...getCreateDraftFromModal(), statementType: value, partyId: '', periodStart: '', periodEnd: '' };
        ...
        global.renderAntdSelect(
            ...
            'bill-create-company',
            global.normalizeList(global.mockData?.companies).map((item) => ({ value: item.id, label: item.name })),
            { placeholder: '选择公司', value: draft.companyId || '' },
            function (value) {
                const nextDraft = { ...getCreateDraftFromModal(), companyId: value || '', partyId: '', periodStart: '', periodEnd: '' };
                openCreateBillModal(nextDraft);
            }
        );

        const partyOptions = draft.statementType === 'supplier'
            ? global.normalizeList(global.mockData?.suppliers).map((item) => ({ value: item.id, label: item.name }))
            : global.normalizeList(global.mockData?.customers).map((item) => ({ value: item.id, label: item.name }));
        global.renderAntdSelect(
            'bill-create-party-container',
            'bill-create-party',
            partyOptions,
            { placeholder: draft.statementType === 'supplier' ? '选择供应商' : '选择客户', value: draft.partyId || '' },
            function (value) {
                const nextDraft = { ...getCreateDraftFromModal(), partyId: value || '', periodStart: '', periodEnd: '' };
                openCreateBillModal(nextDraft);
            }
        );
    }
    ...
    function renderCreateFormSelects(draft) {
        ...
        global.renderAntdSelect(
            ...
            partyOptions,
            {
                placeholder: draft.statementType === 'supplier'
                    ? '选择供应商'
                    : draft.statementType === 'customer'
                        ? '选择客户'
                        : '请先选择对账类型',
            ...
            <div class="bills-route-shell">
                <div class="bills-route-header">
                    <div class="bills-route-title">
                        <span class="text-sm text-gray-500">对账单系统 / 新增对账单</span>
                        <h2 class="text-2xl font-bold text-gray-800">新增对账单</h2>
                        <p>填写基础条件后，系统会自动匹配来源单据并生成正式对账单。</p>
                    </div>
                    <div class="bills-route-actions">
                        <button id="bill-create-back-btn" type="button" class="bills-outline-button">返回对账单列表</button>
                    </div>
                ...
                <div class="bills-route-card">
                    <div class="bills-route-card-body space-y-4">
                        ${buildBillsRouteIntroCard('fa-file-text-o', '新增对账单', '页面布局和信息密度已与出货模块统一，便于连续录入和核对。')}
                        <div class="bills-modal-form-grid">
                            <div class="bills-field">
                                <label>对账类型</label>
                                <div id="bill-create-type-container"></div>
    ...
    function renderBillsViewRoute(statementId, options) {
        ...
        section.innerHTML = `
            <div class="bills-route-shell">
                ...
                ${buildBillsRouteIntroCard('fa-table', '对账单快照', '这里展示当前账期的来源单据、往期欠款和付款记录，便于直接核对与导出。')}
                <div class="bills-route-card">
                    <div class="bills-route-card-body">
                        <div class="bills-route-toolbar">
                            <div id="bill-view-status-container" class="min-w-[220px]"></div>
    ...
    function buildStatementPreviewMarkup(statement) {
        const detailRows = global.normalizeList(statement.details).map((detail, index) => `
            <tr>
                <td>${index + 1}</td>
                <td>${global.escapeHTML(formatBillDateOnly(detail.bizDate))}</td>
                <td>${global.escapeHTML(detail.sourceNo || '-')}</td>
    ...
    function normalizeOccupiedContextEntries(occupiedEntries) {
        return global.normalizeList(occupiedEntries)
            .map((entry) => {
                const liveStatement = entry.statement || getStatementRecords().find((item) => item.id === entry.statementId) || null;
                const statementId = String(liveStatement?.id || entry.statementId || '').trim();
                const sourceNos = Array.from(new Set(
                    global.normalizeList(entry.matchedSources || entry.sourceNos)
                        .map((item) => {
                ...
                return {
                    statementId,
                    partyNameSnapshot: liveStatement?.partyNameSnapshot || entry.partyNameSnapshot || '-',
    ...
    function getOccupiedViewNavigation(statementId) {
        ...
        const index = entries.findIndex((entry) => entry.statementId === statementId);
        if (index === -1) {
            return null;
        }
    ...
    function reopenOccupiedStatementsModal(activeStatementId) {
        ...
        openOccupiedStatementsModal(context.formData || {}, context.entries, activeStatementId || context.activeStatementId || '');
        return true;
    }

    function openOccupiedStatementsModal(formData, occupiedEntries, activeStatementId) {
        ...
        const uniqueSourceCount = new Set(normalizedEntries.flatMap((entry) => entry.sourceNos)).size;
        const statementCount = normalizedEntries.length;
        const currentActiveStatementId = activeStatementId || state.occupiedViewContext?.activeStatementId || normalizedEntries[0].statementId;
            ...
            <div class="space-y-3 text-sm text-gray-700">
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto pr-1">
                    ${normalizedEntries.map((entry, index) => {
                        const isActive = entry.statementId === currentActiveStatementId;
    ...
    function renderBillsViewRoute(statementId, options) {
        ...
        if (!statement) {
            ...
            `;
            document.getElementById('bill-view-missing-back-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list', tab: returnTab }));
            document.getElementById('bill-view-missing-occupied-btn')?.addEventListener('click', () => {
                reopenOccupiedStatementsModal(statementId);
            });
            return;
        }
        ...
        document.getElementById('bill-view-back-btn')?.addEventListener('click', () => navigateToBillsRoute({ type: 'list', tab: returnTab }));
        document.getElementById('bill-view-back-occupied-btn')?.addEventListener('click', () => {
            reopenOccupiedStatementsModal(statement.id);
        });
        document.getElementById('bill-view-prev-occupied-btn')?.addEventListener('click', () => {
            if (!occupiedNav?.prevEntry) return;
            const prevStatement = getStatementRecords().find((item) => item.id === occupiedNav.prevEntry.statementId);
        ...
        document.getElementById('bill-view-next-occupied-btn')?.addEventListener('click', () => {
            if (!occupiedNav?.nextEntry) return;
            const nextStatement = getStatementRecords().find((item) => item.id === occupiedNav.nextEntry.statementId);
    ...
    function persistStatementRecordWithViewContext(statement, options) {
        const bills = global.normalizeList(global.mockData?.bills);
        global.mockData.bills = bills.map((item) => item.id === statement.id ? statement : item);
        if (typeof global.saveMockData === 'function') {
            global.saveMockData();
        }
        updateBillsTableOverride();
        ...
        if (options?.reopenView || isViewingCurrentStatement) {
            openStatementViewModal(statement, {
                returnTab: options?.returnTab || state.activeViewReturnTab || statement.statementType,
    ...
    function openStatementConfirmExportModal(statement, options) {
        const shouldUpdateStatus = normalizeBillStatus(statement.status) === 'pending_check' && !options?.skipStatusConfirm;
        ...
        openBillsModal({
            ...
            rightButtons: [
                ...
                },
                {
                    ...
                    className: 'bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300',
                    onClick: function () {
                        const shouldExportExcel = !!document.getElementById('bill-confirm-export-excel')?.checked;
                        const shouldExportPdf = !!document.getElementById('bill-confirm-export-pdf')?.checked;
    ...
    function buildStatementPreviewMarkup(statement, options) {
        ...
        const taxRate = Number.isFinite(normalizedTaxRate) && normalizedTaxRate > 0 ? normalizedTaxRate : 1;
                ...
                <div class="bills-modal-section">
                    ...
                    <div class="bills-modal-section-body overflow-x-auto">
                        ...
                        <table class="bills-inline-table">
                            ...
                            <tbody>
                                ${detailRows || '<tr><td colspan="10" class="text-center text-gray-400">暂无明细</td></tr>'}
                                ${detailSummaryRows}
                            </tbody>
                        </table>
                    </div>
        ...
        openBillsModal({
            ...
            rightButtons: [
                ...
                },
                {
                    ...
                    onClick: function () {
                        closeBillsModal();
                        const nextStatement = { ...statement, status: 'cancelled', updatedAt: global.getLocalISOString() };
                        saveStatementRecord(nextStatement, true);
                        if (typeof global.addLog === 'function') {
                            const statementTypeLabel = getBillsMeta(nextStatement.statementType).label;
                            const periodText = formatStatementPeriod(nextStatement.periodStart, nextStatement.periodEnd);
                            global.addLog(
                                'cancel',
                                'bill',
                                nextStatement.id,
    ...
    function renderBillsViewRoute(statementId, options) {
        ...
        document.getElementById('bill-view-tax-summary-toggle')?.addEventListener('change', (event) => {
            const checked = !!event.target.checked;
            getStatementViewOptions(statement.id).showTaxSummary = checked;
            const taxSummaryRow = document.getElementById('bill-view-tax-summary-row');
            if (taxSummaryRow) {
    ...
    function renderBillsViewRoute(statementId, options) {
        ...
        if (!statement) {
            section.innerHTML = `
                <div class="bills-route-shell">
                    <div class="bills-route-header">
                        <div class="bills-route-title">
                            <button id="bill-view-missing-up-btn" type="button" class="bills-outline-button mb-3">
            ...
            `;
            document.getElementById('bill-view-missing-up-btn')?.addEventListener('click', () => {
        ...
        section.innerHTML = `
            <div class="bills-route-shell">
                <div class="bills-route-header">
                    <div class="bills-route-title">
                        <button id="bill-view-up-btn" type="button" class="bills-outline-button mb-3">
                            <i class="fa fa-arrow-left"></i>
                            <span>返回上一级</span>
                        </button>
                        <span class="text-sm text-gray-500">对账单系统 / 查看对账单</span>
        ...
        document.getElementById('bill-view-up-btn')?.addEventListener('click', () => {
    ...
    function renderBillsCreateRoute(draft) {
        ...
        document.getElementById('bill-create-up-btn')?.addEventListener('click', () => {
            const previousRoute = state.previousBillsRoute;
    ...
    function mountBillsViewFloatingActions() {
        ...
        root.style.cssText = 'position:fixed;right:max(16px, env(safe-area-inset-right));bottom:max(16px, env(safe-area-inset-bottom));z-index:90;display:flex;align-items:center;gap:12px;';
        root.innerHTML = `
            <button id="bill-view-register-payment-btn" type="button" class="bills-outline-button">登记付款</button>
            <button id="bill-view-confirm-export-btn" type="button" class="bg-primary hover:bg-primary-dark text-white px-6 py-2 rounded-lg transition-all-300">确认并导出</button>
        `;
    ...
    function bindBillsTableEvents() {
        ...
        tbody.addEventListener('click', (event) => {
            ...
            if (!button) return;
            const record = getStatementRecords().find((item) => item.id === button.dataset.id);
            if (!record) return;
            ...
            if (button.dataset.action === 'view') {
                ...
                openBillsModal({
                    title: '导出对账单',
                    content: '<p class="text-sm text-gray-600">请选择要导出的文件格式。</p>',
                    rightButtons: [
                        { label: '取消', className: 'bills-outline-button', onClick: closeBillsModal },
                        { label: 'Excel', className: 'bills-outline-button', onClick: function () { exportStatementAsExcel(record); closeBillsModal(); } },
                        { label: 'PDF', className: 'bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg transition-all-300', onClick: function () { exportStatementAsPdf(record); closeBillsModal(); } }
                    ]
                });
    ...
    try {
        normalizeBillsSectionCopy = normalizeBillsSectionCopy;
        initBillFilters = initBillFiltersOverride;
        updateBillsTable = updateBillsTableOverride;
        renderBillsTable = updateBillsTableOverride;
        bindBillTabEvents = bindBillTabEventsOverride;
    } catch (error) {
        console.warn('Bills module global override warning:', error);
    }
