(function initStockModule(global) {
  ...
  function createSelectNoDataNode() {
    if (typeof window.createAntdEmptyNode === "function") {
      const emptyNode = window.createAntdEmptyNode("暂无数据", {
  ...
  function syncSupplierFilterOption(supplierId, supplierName) {
    ...
    const existingOption = Array.from(filterSupplier.options).find(
  ...
  async function deleteStockMovement(recordId) {
    ...
    addLog(
      ...
      "stock_movement",
      record.productName,
      `删除${record.type === "inbound" ? "进货" : "出货"}记录，回滚数量：${record.quantity}`,
    );
  ...
  function renderStockMovementTable(filter) {
    ...
    if (filter === "inbound") {
      filteredData = stockMovementData.filter(
        (record) => record.type === "inbound",
      );
      emptyColspan = 7;
    ...
    paginatedData.forEach((record) => {
      ...
      const quantityClass =
        record.type === "inbound" ? "text-green-600" : "text-blue-600";
